...
def match_materials_project_reference(
    ...
    min_experimental_snr: float = 3.0,
) -> Dict[str, Any]:
    """Compare an experiment with one uploaded Materials Project XRD JSON.

    Outputs are descriptive reference-match metrics, NOT phase fraction or purity.
    Only the uploaded MP reference is used.
    """
    peaks, _ = detect_peaks(df)
    ...
    if rows.empty:
        return {
            "coverage": 0.0, "matched_count": 0, "reference_count": 0,
            "mean_error_deg": np.nan, "matches": pd.DataFrame(),
        }
    ...
    for _, r in rows.iterrows():
        ...
        if exp:
            nearest = min(exp, key=lambda p: abs(p.pos - tt))
    ...
    return {
        ...
        "matched_count": int(sum(bool(x) for x in [r["Matched"] for r in records])),
        "reference_count": int(len(records)),
        "mean_error_deg": float(np.mean(errors)) if errors else np.nan,
